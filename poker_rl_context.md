# Poker RL Project Context

This document provides the full context, directory structure, and source code for the Poker Reinforcement Learning (RL) project.
...
Poker_RL/
├── .gitignore
├── agent.py            # Neural network architecture (Actor-Critic)
├── poker_env.py        # Core Texas Hold'em game engine
├── requirements.txt    # Project dependencies
├── test_average.py     # Script to simulate 1000 games and average results
├── test_game.py        # Script to simulate and print a single sample game
├── train.py            # Main training script (PPO/Actor-Critic with self-play)
└── vectorized_env.py   # Wrapper for parallel multi-environment training
```
...
class PokerEnv:
    ...
    def _get_state(self, player_idx):
        ...
        # Combine everything into a single 119-dimension array
        state = np.concatenate([
            hand_vec,           # 52 dims
            board_vec,          # 52 dims
            stage_vec,          # 4 dims
            pos_vec,            # 6 dims (for 6 players)
...
def train(num_envs=32, target_episodes=200000, lr=5e-5, gamma=0.99):
    ...
    # Warm-starting logic for expanding state dimensions
    if os.path.exists(villain_agent):
        ...
        try:
            ...
            print(f"Failed to load Hero:", e)

    # Self-play / Historical League setup
    ...
    # Training Loop (Omitted for brevity, standard A2C with parallel envs)
    # ...
```
